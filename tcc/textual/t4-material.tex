...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}p{0.42\textwidth}@{\hspace{6pt}}X@{}}
        \hline
        \textbf{Parâmetro} & \textbf{Valor/controle} \\
        \hline
        Modelo do LLM & Google Gemini / \texttt{gemini-2.5-flash} \\
        Temperatura do LLM & \(0{,}0\) (baixa estocasticidade; \texttt{LLM\_\allowbreak TEMPERATURE=0.0}) \\
        Top-$k$ do RAG (Condição A) & \(k=4\) trechos recuperados por consulta (\texttt{RAG\_TOP\_\allowbreak K=4}) \\
...
Com a metodologia de avaliação definida, o próximo capítulo detalha as decisões de arquitetura e implementação do sistema.