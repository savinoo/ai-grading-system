...
\begin{enumerate}
    ...
    \item Disponibilizar um conjunto de \textit{endpoints} de revisão via API para que o professor possa revisar e validar as notas atribuídas pelo sistema, assegurando coerência com os objetivos acadêmicos definidos previamente.
...
Este trabalho está organizado em sete capítulos. O Capítulo~2 apresenta a fundamentação teórica, abordando conceitos de Inteligência Artificial, modelos de linguagem avançados (LLMs), técnicas de recuperação aumentada por geração (RAG) e sistemas multiagentes. O Capítulo~3 discute os trabalhos relacionados, por meio de uma revisão sistemática da literatura sobre correção automatizada de textos. O Capítulo~4 descreve a metodologia adotada, incluindo o processo de correção proposto, as ferramentas utilizadas e o método de avaliação dos resultados. O Capítulo~5 detalha o desenvolvimento do sistema, contemplando a arquitetura geral, o \textit{pipeline} de correção multiagente e a modelagem do banco de dados. O Capítulo~6 apresenta os resultados obtidos e as discussões pertinentes. Por fim, o Capítulo~7 traz as conclusões e sugestões de trabalhos futuros.
