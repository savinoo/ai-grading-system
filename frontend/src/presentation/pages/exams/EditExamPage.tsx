...
export const EditExamPage: React.FC = () => {
  ...
  }, [examUuid, isEditMode, loadGradingCriteria, loadTeacherClasses, loadExamDetails, loadExamCriteria, loadAttachments, loadQuestionsByExam]);
  ...
  // Carregar estudantes quando turma for selecionada
  useEffect(() => {
    if (classUuid && currentStep === 2) {
      loadStudentsByClass(classUuid);
    }
  }, [classUuid, currentStep, loadStudentsByClass]);
  ...
  const handleNextStep = async () => {
    if (currentStep === 1) {
      ...
      // Salvar automaticamente APENAS no modo de criação
      if (!isEditMode) {
  ...
  const handleCancel = () => {
    // No modo de edição, apenas volta sem excluir
    if (isEditMode) {
      navigate('/dashboard/exams');
      return;
    }
    
    // No modo de criação, abre o modal de confirmação de exclusão
  ...
  return (
    <DashboardLayout>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 p-4 z-50">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <button
            ...
            disabled={isLoading}
          >
            <span className="material-symbols-outlined text-lg">{isEditMode ? 'arrow_back' : 'close'}</span>
            {isEditMode ? 'Voltar' : 'Cancelar e Sair'}
          </button>
      ...
      {/* Modal de Confirmação de Exclusão (apenas no modo de criação) */}
