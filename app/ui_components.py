"""
UI Components — Streamlit visual components for the grading pipeline.
Adapted from main branch for tcc architecture.
"""
import streamlit as st


def setup_page():
    st.set_page_config(page_title="CorretumAI — Correção Automatizada (TCC)", layout="wide", page_icon="📝")
...
def render_student_report(student_data):
    """Renderiza o relatório detalhado de um aluno."""
    ...
    for idx, q_res in enumerate(student_data['details']):
        ...
        with st.expander(f"{icon} Questão {idx+1} | Nota: {q_grade:.1f} | {q_res['question_text'][:60]}..."):
            ...
            with c_right:
                ...
                t_overview, t_rag, t_agents, t_arbiter = st.tabs(
                    ["📝 Resultado", "📚 Contexto (RAG)", "🤖 Agentes (Thinking)", "⚖️ Árbitro"]
                )
                ...
                with t_rag:
                    rag_ctx = full_state.get('rag_contexts', [])
                    if rag_ctx:
...
def render_global_kpis(df_res):
    """Renderiza KPIs globais da turma."""
    ...
    col_kpi1.metric("Média Geral", f"{avg_grade:.1f}",
                    delta=f"{avg_grade-6.0:.1f} vs Meta" if avg_grade >= 6 else f"{avg_grade-6.0:.1f}")
    col_kpi2.metric("Maior Nota", f"{max_grade:.1f}")
    col_kpi3.metric("Aprovação", f"{pass_rate*100:.0f}%")


def render_class_ranking(df_res):
    """Renderiza ranking da turma."""
    st.dataframe(
