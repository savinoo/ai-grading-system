...
class StudentTracker:
    ...
    Thresholds are configurable via .env (GAP_THRESHOLD, STRENGTH_THRESHOLD).
    """
    ...
    def _identify_gaps(self, profile: StudentProfile) -> list[LearningGap]:
        ...
        A gap is a criterion where avg score < GAP_THRESHOLD (default 6.0).
        """
        gaps = []
        ...
        for crit_name, scores in criterion_tracker.items():
            ...
            if avg_score < self.gap_threshold:
    ...
    def _identify_strengths(self, profile: StudentProfile) -> list[Strength]:
        ...
        Strength threshold configurable via STRENGTH_THRESHOLD (default 8.0).
        """
        strengths = []
        criterion_tracker: dict[str, list[float]] = {}
        ...
        for crit_name, scores in criterion_tracker.items():
            ...
            if avg_score >= self.strength_threshold:
    ...
    def export_profile(self, student_id: str) -> dict | None:
