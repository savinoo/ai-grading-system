...
class GetAttachmentByUuidController(ControllerInterface):
    """
    Controller responsável por buscar um anexo por UUID.
    """
    ...
    def handle(self, http_request: HttpRequest) -> HttpResponse:
        """
        Processa a requisição de busca de anexo por UUID.
        
        Args:
            http_request: Requisição HTTP
            
        Returns:
            HttpResponse: Resposta HTTP com os dados do anexo
        ...
        self.__logger.debug(
            "Handling get attachment by uuid request from caller: %s",
        ...
        try:
            ...
            # Busca anexo
            attachment = asyncio.run(self.__service.get_by_uuid(db, uuid))

            self.__logger.info("Anexo encontrado: %s", uuid)
            ...
            self.__logger.error(
                "Erro de banco de dados ao buscar anexo: %s",
                sql_err,
                exc_info=True
            )
            raise HTTPException(
            ...
            self.__logger.error(
                "Erro inesperado ao buscar anexo: %s",
                e,
                exc_info=True
            )
            raise HTTPException(
                status_code=500,
                detail={"error": "Erro inesperado ao buscar anexo"}
            ) from e
